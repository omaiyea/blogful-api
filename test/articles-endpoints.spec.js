...
describe('Articles Endpoints', function(){
    ...
    describe('GET /api/articles', () => {
        ...
        context(`Given an XSS attack on all the articles`, () => {
    ...
    describe.only(`POST /api/articles`, () => {
        it(`creates an article, responding with 201 and the new article`, function(){
            ...
            return supertest(app)
             ...
             .send(newArticle)
             .expect(201)
             .expect(res => { //test data is as expected
                 expect(res.body.title).to.eql(newArticle.title)
                 expect(res.body.style).to.eql(newArticle.style)
                 expect(res.body.content).to.eql(newArticle.content)
                 expect(res.body).to.have.property('id')
                 expect(res.headers.location).to.eql(`/api/articles/${res.body.id}`)
                 const expected = new Date().toLocaleString('en', {timeZone: 'UTC'}) // remove miliseconds
                 const actual = new Date(res.body.date_published).toLocaleString()
                 expect(actual).to.eql(expected)
             })
             .then(postRes => //test post using articles/:articleId endpoint
                supertest(app)
